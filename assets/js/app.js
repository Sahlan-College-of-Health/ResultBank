...
function renderLockedStudentPortal(student) {
  app.innerHTML = `
    <div class="public-page-shell">
      <header class="public-topbar">
        <div class="brand">
      ...
      <main class="public-main">
        <section class="public-content-card">
          ...
          <section class="card section result-access-locked-card">
            <h2>Result Access Locked</h2>
            <p>Your academic result is currently unavailable because your school-fee clearance has not been confirmed.</p>
            <p>Please contact the College Administration after completing the required payment.</p>
            <div class="message warning" style="margin-top:14px">
              Result viewing and carry-over registration will become available after the Administrator restores your result access.
            </div>
          </section>
        </section>
      </main>
...
async function renderStudentPortal(studentId) {
  ...
  if (student.resultLocked === true) {
    renderLockedStudentPortal(student);
    return;
  }
    ...
    button.addEventListener("click", () => {
      ...
      area.innerHTML = rows.length ? `
        <div class="result-toolbar no-print">
          ...
          <span class="status-badge active">View Only</span>
        </div>
      ...
      <div class="layout">
        <aside id="sidebar" class="sidebar">
          ...
          <nav class="nav-list">
            ...
            ${navButton("bulkSessionResults", "Bulk Session Results")}
...
async function navigate(view) {
  ...
  } else if (view === "bulkSessionResults") {
  ...
  const content = `
    ...
    <section class="section">
      ...
      <div class="grid quick-grid">
        ...
        ${quickAction("studentAccounts", "Student Login Accounts", "Create personal passwords and view generated login passwords.")}
        ${quickAction("resultAccess", "Result Access Control", "Lock or unlock result viewing for school-fee clearance.")}
        ${quickAction("bulkSessionResults", "Bulk Session Results", "Print or save every student session result in one job.")}
  ...
  dashboardLayout(`
    ...
    <section class="card">
      <div class="import-note">
        Newly generated passwords are stored in an admin-only credentials collection and are also downloaded as CSV.
        Existing Firebase passwords created before this version cannot be recovered. If a student later changes the password,
        the value shown here remains the original generated password.
      </div>
...
async function loadStudentAccountStatus() {
  ...
  try {
    const [studentSnapshot, credentialSnapshot] = await Promise.all([
      getDocs(collection(db, "students")),
      getDocs(collection(db, "studentCredentials"))
    ]);

    const credentialMap = new Map(
      credentialSnapshot.docs.map(item => [item.id, item.data()])
    );
      ...
      <table>
        ...
        <tbody>
          ${students.map(student => {
            const credential = credentialMap.get(student.id);
            const savedPassword = credential?.generatedPassword || "";
            return `
              <tr>
                ...
                <td>
                  ${savedPassword
                    ? `<code class="admin-password-value">${escapeHtml(savedPassword)}</code>`
                    : `<span class="muted-text">${student.authUid ? "Not recorded (older account)" : "—"}</span>`}
                </td>
...
async function renderResultAccessControl() {
  dashboardLayout(`
    <section class="section-head">
      <div>
        <h3>Result Access Control</h3>
        <p>Lock or unlock student result access based on school-fee clearance.</p>
      </div>
    </section>
    ...
    <section class="card">
      <div class="form-grid">
        <div class="form-group">
          <label for="resultAccessSearch">Search Student</label>
          <input id="resultAccessSearch" placeholder="Name or matric number">
        </div>
        <div class="form-group">
          <label for="resultAccessFilter">Access Status</label>
          <select id="resultAccessFilter">
      ...
      <div id="resultAccessMessage"></div>
      <div id="resultAccessArea" class="placeholder" style="margin-top:16px">Loading students...</div>
    </section>
  `);

  document.getElementById("resultAccessSearch").addEventListener("input", loadResultAccessControl);
  document.getElementById("resultAccessFilter").addEventListener("change", loadResultAccessControl);
  await loadResultAccessControl();
}

async function loadResultAccessControl() {
  const area = document.getElementById("resultAccessArea");
  if (!area) return;

  try {
    const snapshot = await getDocs(collection(db, "students"));
    const search = (document.getElementById("resultAccessSearch")?.value || "").trim().toLowerCase();
    const filter = document.getElementById("resultAccessFilter")?.value || "";

    let students = snapshot.docs.map(item => ({ id: item.id, ...item.data() }));
    students = students
      .filter(student => {
        const haystack = `${student.name || ""} ${student.matricNumber || ""}`.toLowerCase();
        return !search || haystack.includes(search);
      })
      .filter(student => {
        if (filter === "locked") return student.resultLocked === true;
        if (filter === "allowed") return student.resultLocked !== true;
        return true;
      })
      .sort((a, b) => String(a.name || "").localeCompare(String(b.name || "")));
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Matric Number</th>
            <th>Department</th>
            <th>School-Fee / Result Status</th>
            <th>Action</th>
          </tr>
        </thead>
        <tbody>
          ${students.map(student => {
            const locked = student.resultLocked === true;
            return `
              <tr>
                <td>${escapeHtml(student.name || "")}</td>
                <td>${escapeHtml(student.matricNumber || "")}</td>
                <td>${escapeHtml(student.department || "")}</td>
                <td>
                  <span class="status-badge ${locked ? "closed" : "active"}">
                    ${locked ? "Locked — Fees Not Cleared" : "Allowed — Cleared"}
                  </span>
                </td>
                <td>
                  <button
                    class="${locked ? "primary-btn" : "danger-btn"} result-access-toggle"
                    data-id="${escapeHtml(student.id)}"
                    data-locked="${locked ? "true" : "false"}"
                    data-name="${escapeHtml(student.name || "Student")}">
    ...
    document.querySelectorAll(".result-access-toggle").forEach(button => {
      button.addEventListener("click", () => toggleStudentResultAccess(
        button.dataset.id,
...
async function toggleStudentResultAccess(studentId, currentlyLocked, studentName) {
  const action = currentlyLocked ? "unlock" : "lock";
  const confirmed = confirm(
    `${action === "lock" ? "Lock" : "Unlock"} result access for ${studentName}?`
  );
  if (!confirmed) return;

  const message = document.getElementById("resultAccessMessage");

  try {
    await setDoc(doc(db, "students", studentId), {
      resultLocked: !currentlyLocked,
      resultLockReason: !currentlyLocked ? "School fees not cleared" : "",
      resultAccessUpdatedAt: serverTimestamp(),
      resultAccessUpdatedBy: state.user?.uid || ""
    }, { merge: true });

    message.innerHTML = `<div class="message success">Result access updated successfully.</div>`;
    await loadResultAccessControl();
  } catch (error) {
    message.innerHTML = `<div class="message error">${escapeHtml(error.message)}</div>`;
  }
...
async function renderBulkSessionResults() {
  dashboardLayout(`
    <section class="section-head">
      <div>
        <h3>Bulk Academic Session Results</h3>
        <p>Prepare every student's First and Second Semester result for one academic session in a single print/PDF job.</p>
      </div>
    </section>
    ...
    <section class="card">
      <div class="form-grid">
        <div class="form-group">
          <label for="bulkResultSession">Academic / Result Session</label>
          <select id="bulkResultSession">
            <option value="">Loading sessions...</option>
          </select>
        </div>
        <div class="form-group" style="align-self:end">
          <button id="bulkPrepareBtn" class="primary-btn" type="button">Prepare All Student Results</button>
        </div>
      </div>
      <div class="import-note" style="margin-top:14px">
        Each student is placed on a separate A4 page containing all available First and Second Semester results for the selected session.
        Use the browser print dialog to print or choose <strong>Save as PDF</strong> to save the complete session as one PDF file.
      </div>
      <div id="bulkSessionMessage"></div>
      <div id="bulkSessionPreview" class="placeholder" style="margin-top:16px">Select an academic session.</div>
    </section>
  `);

  const resultSnapshot = await getDocs(collection(db, "results"));
  const sessions = [...new Set(
    resultSnapshot.docs.map(item => String(item.data().resultSession || "").trim()).filter(Boolean)
  )].sort((a, b) => b.localeCompare(a));

  const select = document.getElementById("bulkResultSession");
  select.innerHTML = `<option value="">Select session</option>${sessions.map(session => `
    <option value="${escapeHtml(session)}">${escapeHtml(session)}</option>
  `).join("")}`;

  document.getElementById("bulkPrepareBtn").addEventListener("click", prepareBulkSessionResults);
}

async function prepareBulkSessionResults() {
  const selectedSession = document.getElementById("bulkResultSession")?.value || "";
  const preview = document.getElementById("bulkSessionPreview");
  const message = document.getElementById("bulkSessionMessage");

  if (!selectedSession) {
    message.innerHTML = `<div class="message error">Select an academic session first.</div>`;
    return;
  }

  preview.className = "placeholder";
  preview.textContent = "Preparing session results...";
  message.innerHTML = "";

  try {
    const [studentSnapshot, resultSnapshot] = await Promise.all([
      getDocs(collection(db, "students")),
      getDocs(query(collection(db, "results"), where("resultSession", "==", selectedSession)))
    ]);

    const studentMap = new Map(
      studentSnapshot.docs.map(item => [item.id, { id: item.id, ...item.data() }])
    );

    const groups = new Map();
    resultSnapshot.docs.forEach(item => {
      const result = { id: item.id, ...item.data() };
      const key = result.studentId || safeDocumentId(result.matricNumber || "");
      if (!key) return;
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(result);
    });

    const prepared = [...groups.entries()].map(([studentId, rows]) => {
      const semesterMap = new Map();
      rows.sort((a, b) => resultSortKey(a) - resultSortKey(b)).forEach(result => {
        const semesterKey = `${Number(result.level || 0)}_${String(result.semester || "").trim().toLowerCase()}`;
        semesterMap.set(semesterKey, result);
      });
      const sortedRows = [...semesterMap.values()].sort((a, b) => resultSortKey(a) - resultSortKey(b));
      let student = studentMap.get(studentId);
      if (!student) {
        const first = sortedRows[0] || {};
        student = [...studentMap.values()].find(item => item.matricNumber === first.matricNumber) || {
          id: studentId,
          name: first.studentName || first.name || "Unknown Student",
          matricNumber: first.matricNumber || "",
          sex: first.sex || "",
          department: first.department || "",
          admissionSession: first.admissionSession || "",
          currentLevel: first.level || ""
        };
      }
      return { student, rows: sortedRows };
    }).sort((a, b) => String(a.student.name || "").localeCompare(String(b.student.name || "")));

    if (!prepared.length) {
      preview.className = "placeholder";
      preview.textContent = "No results were found for the selected academic session.";
      return;
    }

    preview.className = "";
    const markup = prepared.map(({ student, rows }) => `
      <article class="official-result session-result compact-a4-result bulk-session-result-page">
        ${officialResultHeader(`${escapeHtml(selectedSession)} ACADEMIC SESSION RESULT`)}
        ${studentIdentitySection(student)}
        ${rows.map(semesterResultBlock).join("")}
        ${yearSummaryBlock(rows)}
        ${signatureSection()}
    ...
    preview.innerHTML = `
      <div class="preview-summary" style="margin-bottom:14px">
        <article><strong>${prepared.length}</strong><span>Students Prepared</span></article>
        <article><strong>${escapeHtml(selectedSession)}</strong><span>Academic Session</span></article>
      </div>
      <div class="action-cell" style="justify-content:flex-start;gap:10px;flex-wrap:wrap">
        <button id="bulkDownloadPdfBtn" class="primary-btn" type="button">Download All as One PDF</button>
        <button id="bulkPrintAllBtn" class="secondary-btn" type="button">Print All</button>
      </div>
      <div class="import-note" style="margin-top:12px">
        The document contains ${prepared.length} A4 page(s), one page per student.
      </div>
    `;

    document.getElementById("bulkDownloadPdfBtn").addEventListener("click", () => {
      downloadBulkResultPdf(markup, selectedSession, prepared.length);
    });

    document.getElementById("bulkPrintAllBtn").addEventListener("click", () => {
      printBulkResultMarkup(markup, selectedSession);
    });
  } catch (error) {
...
async function downloadBulkResultPdf(markup, session, studentCount) {
  if (state.userProfile?.role !== "admin") {
    alert("Only the Administrator is authorized to download student results.");
    return;
  }

  if (typeof window.html2pdf !== "function") {
    alert("The PDF generator did not load. Refresh the page and try again, or use Print All and choose Save as PDF.");
    return;
  }

  const button = document.getElementById("bulkDownloadPdfBtn");
  const originalText = button?.textContent || "Download All as One PDF";
  if (button) {
    button.disabled = true;
  ...
  host.id = "bulkPdfRenderHost";
  host.style.position = "fixed";
  host.style.left = "-10000px";
  host.style.top = "0";
  host.style.width = "194mm";
  host.style.background = "#fff";
  host.innerHTML = `<div id="printOnlyContainer" class="print-only-container print-bulk-session">${markup}</div>`;
  document.body.appendChild(host);

  try {
    await new Promise(resolve => setTimeout(resolve, 500));

    const filename = `SAHLAN-${String(session).replace(/[^a-z0-9]+/gi, "-")}-All-Student-Results.pdf`;
    const options = {
      margin: 0,
      filename,
      ...
      pagebreak: { mode: ["css", "legacy"], before: ".bulk-session-result-page:not(:first-child)" }
    };

    await window.html2pdf().set(options).from(host.querySelector("#printOnlyContainer")).save();

    const message = document.getElementById("bulkSessionMessage");
    if (message) {
      message.innerHTML = `<div class="message success">Downloaded one PDF containing ${studentCount} student result page(s).</div>`;
    }
  } catch (error) {
    console.error("Bulk PDF generation error:", error);
    alert("The PDF could not be generated. Use Print All and choose Save as PDF instead.");
  } finally {
    host.remove();
    if (button) {
      button.disabled = false;
...
async function printBulkResultMarkup(markup, session) {
  if (state.userProfile?.role !== "admin") {
    alert("Only the Administrator is authorized to print or download student results.");
    return;
  }

  const existingFrame = document.getElementById("bulkResultPrintFrame");
  if (existingFrame) existingFrame.remove();

  const printFrame = document.createElement("iframe");
  printFrame.id = "bulkResultPrintFrame";
  ...
  printDocument.write(`
    ...
    <html lang="en">
      <head>
        ...
        <base href="${basePath}">
        <title>SAHLAN ${escapeHtml(session)} Session Results</title>
        <link rel="stylesheet" href="./assets/css/style.css?v=3.1.0">
        <link rel="stylesheet" href="./assets/css/print.css?v=3.1.0">
      </head>
      <body class="printing-result">
        <div id="printOnlyContainer" class="print-only-container print-bulk-session">
          ${markup}
        </div>
      </body>
    </html>
...
async function renderCarryOverApplications() {
  dashboardLayout(`
    <section class="section-head">
      ...
      <button id="printPendingApplicationsBtn" class="primary-btn">
        Print Pending List
      </button>
    </section>
  ...
  printDocument.write(`
    ...
    <html lang="en">
      <head>
        ...
        <link
          rel="stylesheet"
          href="./assets/css/style.css?v=3.1.0"
        >

        <link
          rel="stylesheet"
          href="./assets/css/print.css?v=3.1.0"
        >
      </head>
...
function signatureSection() {
  return `<section class="signature-grid management-signatures registrar-only-signature">
