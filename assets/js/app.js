...
const COLLEGE_CONFIG = {
  health: {
    key: "health",
    name: "SAHLAN COLLEGE OF HEALTH SCIENCE AND TECHNOLOGY, JOS",
    shortName: "SAHLAN COLLEGE OF HEALTH SCIENCE AND TECHNOLOGY",
    logo: "assets/images/logo.png",
    departments: ["CHEW", "MLT"]
  },
  nursing: {
    key: "nursing",
    name: "SAHLAN COLLEGE OF NURSING AND MIDWIFERY",
    shortName: "SAHLAN COLLEGE OF NURSING AND MIDWIFERY",
    logo: "assets/images/nursing-logo.jpg",
    departments: ["NURSING"]
  }
};

function collegeKeyForDepartment(department) {
  return String(department || "").trim().toUpperCase() === "NURSING" ? "nursing" : "health";
}

function getCollegeConfig(value = state.activeCollege) {
  if (value && COLLEGE_CONFIG[value]) return COLLEGE_CONFIG[value];
  if (typeof value === "object" && value) return COLLEGE_CONFIG[collegeKeyForDepartment(value.department)];
  return COLLEGE_CONFIG.health;
}

function collegeMatches(department, college = state.activeCollege) {
  return collegeKeyForDepartment(department) === college;
}

function collegeLabelForDepartment(department) {
  return collegeKeyForDepartment(department) === "nursing"
    ? "Nursing & Midwifery"
    : String(department || "");
}

function loginEmailForMatric(matricNumber) {
  const normalized = String(matricNumber || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  ...
  return `${normalized}@students.sahlan.local`;
}
  ...
  app.innerHTML = `
    <main class="public-home">
      ...
      <section class="public-hero-card">
        ...
        <footer>
          © 2026 ${APP_CONFIG.schoolName} · Professional Edition v4.0
        </footer>
      </section>
...
function openStudentLoginModal() {
      ...
      <section class="student-modal student-login-modal">
        <div class="modal-head">
          <div>
            ...
            <p class="student-login-subtitle">
              Use your matric number and your personal password.
            </p>
          </div>
        ...
        <form id="studentLoginForm">
          ...
          <div class="form-group" style="margin-top:14px">
            <label for="studentLoginPassword">Personal Password</label>
            <input
  ...
  document.getElementById("studentLoginForm").addEventListener("submit", async event => {
    ...
    try {
      const email = loginEmailForMatric(matricNumber);
      ...
      const studentMessages = {
        "auth/invalid-credential": "Incorrect matric number or password.",
        "auth/user-not-found": "No student login account was found.",
        "auth/wrong-password": "Incorrect matric number or password.",
        "auth/too-many-requests": "Too many attempts. Wait briefly and try again.",
        "auth/network-request-failed": "Firebase Authentication could not be reached from this device or network."
      };

      message.innerHTML = `
        <div class="message error">
          ${escapeHtml(studentMessages[error.code] || error.message)}
        </div>
      `;
    }
  });
...
async function loadCounts() {
  const [studentsSnap, resultsSnap, coursesSnap, sessionsSnap] = await Promise.all([
  ...
  const students = studentsSnap.docs.map(d => d.data()).filter(s => collegeMatches(s.department));
  const results = resultsSnap.docs.map(d => d.data()).filter(r => collegeMatches(r.department));
  const courses = coursesSnap.docs.map(d => d.data()).filter(c => collegeMatches(c.department));
  const sessions = sessionsSnap.docs.map(d => d.data()).filter(session => {
    if (session.department) return collegeMatches(session.department);
    return students.some(student => student.admissionSession === session.admissionSession);
  });

  state.counts.students = students.length;
  state.counts.results = results.length;
  state.counts.courses = courses.length;
  state.counts.sessions = sessions.length;
}
  ...
  app.innerHTML = `
    <div class="public-page-shell">
      <header class="public-topbar">
        <div class="brand">
          <img src="${getCollegeConfig().logo}" alt="School logo">
          <div>
            <h1>${getCollegeConfig().name}</h1>
            <small>${getCollegeConfig().key === "nursing" ? "Nursing & Midwifery Result Bank" : "Health Science Result Bank"}</small>
          </div>
        </div>
...
async function loadPublicSessions() {
  ...
  try {
    ...
    snap.docs.forEach(d => {
      const s = d.data();
      if (!collegeMatches(s.department)) return;
...
async function renderStudentPortal(studentId) {
  ...
  if (student.resultLocked === true) {
    const college = getCollegeConfig(student);
    app.innerHTML = `
      <div class="public-page-shell">
        <header class="public-topbar">
        ...
        <main class="public-main"><section class="public-content-card access-locked-card">
          ...
          <h2>Result Access Restricted</h2>
          <p>Your academic result is currently unavailable because your school-fee clearance has not been confirmed by the College Administration.</p>
...
const years = [...new Set(results.map(result => Number(result.level)).filter(Boolean))].sort();
  ...
  app.innerHTML = `
    <div class="public-page-shell">
      <header class="public-topbar">
        <div class="brand">
          <img src="${getCollegeConfig(student).logo}" alt="School logo">
          <div>
            <h1>${getCollegeConfig(student).name}</h1>
            <small>Secure Student Portal</small>
          </div>
        </div>
        <div class="top-actions">
          <button id="changeStudentPasswordBtn" class="secondary-btn">Change Password</button>
          <button id="studentLogoutBtn" class="danger-btn">Logout</button>
        </div>
      </header>
      ...
      <main class="public-main">
        <section class="public-content-card">
          ...
          <section class="section">
            <div class="year-buttons">
              ${years.map(year => `
                <button class="secure-year-btn year-result-btn" data-year="${year}">
                  <strong>Year ${year}</strong>
                  <span>First and Second Semester</span>
                </button>
              `).join("")}
              <button id="secureTranscriptBtn" class="year-result-btn transcript-highlight">
                <strong>Complete Transcript</strong>
                <span>All available semesters</span>
              </button>
            </div>
          ...
          <section class="card section">
            <div class="section-head">
              <div>
                ...
                <p>Register the courses you will offer for the current academic session.</p>
              </div>
              <button id="openCourseRegistrationBtn" class="secondary-btn">View / Register Courses</button>
            </div>
            <div id="courseRegistrationArea" style="display:none; margin-top:14px;"></div>
          </section>

          <section class="card section">
            <div class="section-head">
              <div>
                <h3>Apply to Rewrite Carry-Over Courses</h3>
                <p>Select outstanding courses and submit your application.</p>
              </div>
            </div>
            <div id="studentCarryApplicationArea"></div>
          </section>

          <section id="secureStudentResultArea" class="section"></section>
        </section>
      </main>

      <div id="studentPasswordModalHost"></div>
    </div>
  `;
  ...
  document.querySelectorAll(".secure-year-btn").forEach(button => {
    button.addEventListener("click", () => {
      ...
      const rows = results
        .filter(result => Number(result.level) === year)
        .sort((a, b) => resultSortKey(a) - resultSortKey(b));

      area.innerHTML = rows.length ? `
        <div class="result-toolbar no-print">
          <h3>Year ${year} Result</h3>
          <span class="student-view-only-badge">View Only</span>
        </div>
        <article class="official-result session-result">
          ${officialResultHeader(`YEAR ${year} COMPLETE SESSION RESULT`, student)}
          ${studentIdentitySection(student)}
          ${rows.map(semesterResultBlock).join("")}
          ${yearSummaryBlock(rows)}
      ...
      <article class="official-result">
        ...
        ${levels.map(level => `
          <section class="transcript-year">
            <h3>YEAR ${level}</h3>
            ${results
              .filter(result => Number(result.level) === level)
              .sort((a, b) => resultSortKey(a) - resultSortKey(b))
              .map(semesterResultBlock)
              .join("")}
          </section>
        `).join("")}
        ${yearSummaryBlock(results)}
        ${signatureSection()}
      </article>
    `;
...
function openStudentPasswordModal() {
  const host = document.getElementById("studentPasswordModalHost");

  host.innerHTML = `
    <div class="modal-backdrop" id="studentPasswordBackdrop">
      <section class="student-modal" style="max-width:460px">
        <div class="modal-head">
          <h3>Change Password</h3>
          <button id="closeStudentPassword" class="icon-btn" type="button">✕</button>
        </div>
        <form id="studentPasswordForm">
          <div class="form-group">
            <label for="newStudentPassword">New Password</label>
            <input id="newStudentPassword" type="password" minlength="8" required>
          </div>
          <div class="form-group" style="margin-top:14px">
            <label for="confirmStudentPassword">Confirm Password</label>
            <input id="confirmStudentPassword" type="password" minlength="8" required>
          </div>
          <button class="primary-btn" type="submit" style="width:100%;margin-top:18px">
            UPDATE PASSWORD
          </button>
  ...
  const close = () => host.innerHTML = "";
  document.getElementById("closeStudentPassword").addEventListener("click", close);
  document.getElementById("studentPasswordBackdrop").addEventListener("click", event => {
    if (event.target.id === "studentPasswordBackdrop") close();
  });

  document.getElementById("studentPasswordForm").addEventListener("submit", async event => {
    event.preventDefault();
    const password = document.getElementById("newStudentPassword").value;
    const confirmation = document.getElementById("confirmStudentPassword").value;
    const message = document.getElementById("studentPasswordMessage");

    if (password !== confirmation) {
      message.innerHTML = `<div class="message error">The passwords do not match.</div>`;
      return;
    }

    try {
      await updatePassword(auth.currentUser, password);
      message.innerHTML = `<div class="message success">Password updated successfully.</div>`;
    } catch (error) {
...
async function renderStudentCourseRegistration(student, results) {
  ...
  try {
    ...
    const carrySnapshot = await getDocs(
      query(
        collection(db, "carryOverApplications"),
        where("studentId", "==", student.id)
      )
    );

    const carryOverCourses = carrySnapshot.docs
      .flatMap(item => item.data().courses || []);

    const carryOverLoad = calculateRegistrationCredits(carryOverCourses);

    const newCourseCreditCap = Math.max(
      0,
      maximumCreditLoad - carryOverLoad
    );
    ...
    area.innerHTML = `
      <div class="registration-meta-grid">
        ...
        <div>
          <span>Carry-Over Load Already Committed</span>
          <strong>${carryOverLoad} Units</strong>
        </div>

        <div>
          <span>Available for New Courses</span>
          <strong>${newCourseCreditCap} Units</strong>
        </div>
      </div>

      <div class="credit-load-box">
        <strong>New Courses Selected:</strong>
        <span id="selectedCreditLoad">0</span>
        /
        <span>${newCourseCreditCap}</span>
        Units
        ${carryOverLoad ? `<br><small>Plus ${carryOverLoad} carry-over unit(s) already committed. Combined total may not exceed ${maximumCreditLoad} units.</small>` : ""}
      </div>
    ...
    const load = () => {
      ...
      if (selected > newCourseCreditCap) {
        message.innerHTML = `
          <div class="message error">
            You have selected ${selected} credit units.
            Only ${newCourseCreditCap} unit(s) remain available for new
            courses in Year ${currentLevel}
            ${escapeHtml(student.department)}
            (maximum ${maximumCreditLoad} units, minus
            ${carryOverLoad} carry-over unit(s) already committed).
            Please deselect some courses before you can save.
          </div>
        `;
      } else if (selected === 0) {
        message.innerHTML = "";
      } else if (selected < newCourseCreditCap) {
        message.innerHTML = `
          <div class="message">
            You have selected ${selected} of
            ${newCourseCreditCap} available credit units.
            You may select more courses, or submit now.
          </div>
        `;
      } else {
        message.innerHTML = "";
      ...
      if (submitButton) {
        submitButton.disabled = selected === 0 || selected > newCourseCreditCap;
      }
    };
...
function printCourseRegistrationForm(
  ...
  container.innerHTML = `
    <article class="official-result course-registration-print">
      ...
      ${courseRegistrationSignatureSection()}
...
async function renderStudentCarryApplication(student, latestResult) {
  ...
  const courses = latestResult.unresolvedCarryOvers || [];
  ...
  const pendingApplication = existingSnapshot.docs
    .map(item => ({ id: item.id, ...item.data() }))
  ...
  if (pendingApplication) {
    area.innerHTML = `
      <div class="message success">
        Your application is pending review. Submitted courses:
        ${pendingApplication.courses.map(course => escapeHtml(course.courseCode)).join(", ")}.
      </div>
    `;
    return;
  }
  ...
  if (!courses.length) {
    area.innerHTML = `<div class="placeholder">You currently have no outstanding carry-over courses.</div>`;
    return;
  }

  area.innerHTML = `
    <form id="carryApplicationForm">
      <div class="carry-application-list">
        ${courses.map((course, index) => `
          <label class="carry-application-item">
            <input
              type="checkbox"
              name="carryCourse"
              value="${index}"
      ...
      <button class="primary-btn" type="submit" style="margin-top:14px">
        Submit Rewrite Application
      </button>
      <div id="carryApplicationMessage"></div>
    </form>
  `;

  document.getElementById("carryApplicationForm").addEventListener("submit", async event => {
    event.preventDefault();

    const selectedIndexes = [...document.querySelectorAll('input[name="carryCourse"]:checked')]
      .map(input => Number(input.value));

    const message = document.getElementById("carryApplicationMessage");

    if (!selectedIndexes.length) {
      message.innerHTML = `<div class="message error">Select at least one course.</div>`;
      return;
    }

    const selectedCourses = selectedIndexes.map(index => courses[index]);
    const applicationId = safeDocumentId(
      `${student.id}_${latestResult.resultSession || "current"}`
    );

    try {
      await setDoc(doc(db, "carryOverApplications", applicationId), {
        studentId: student.id,
        studentName: student.name,
        matricNumber: student.matricNumber,
        department: student.department,
        currentLevel: student.currentLevel,
        resultSession: latestResult.resultSession || "",
        courses: selectedCourses,
        status: "Pending",
        appliedAt: serverTimestamp()
      });

      message.innerHTML = `<div class="message success">Application submitted successfully.</div>`;
      await renderStudentCarryApplication(student, latestResult);
    } catch (error) {
      message.innerHTML = `<div class="message error">${escapeHtml(error.message)}</div>`;
    }
  });
...
function dashboardLayout(content) {
  app.innerHTML = `
    <div class="page-shell">
      <header class="topbar">
        ...
        <div class="top-actions">
          <label class="college-switcher">
            <span>College</span>
            <select id="adminCollegeSwitcher">
              <option value="health" ${state.activeCollege === "health" ? "selected" : ""}>Health Science & Technology</option>
              <option value="nursing" ${state.activeCollege === "nursing" ? "selected" : ""}>Nursing & Midwifery</option>
            </select>
          </label>
      ...
      <div class="layout">
        <aside id="sidebar" class="sidebar">
          ...
          <nav class="nav-list">
            ...
            ${navButton("studentAccounts", "Student Login Accounts")}
            ${navButton("resultAccess", "Result Access Control")}
            ${navButton("bulkResults", "Bulk Session Results")}
            ${navButton("results", "Manage Results")}
            ${navButton("carryApplications", "Carry-Over Applications")}
  ...
  document.getElementById("adminCollegeSwitcher")?.addEventListener("change", async (event) => {
    state.activeCollege = event.target.value;
...
async function navigate(view) {
  ...
  } else if (view === "studentAccounts") {
    await renderStudentAccounts();
  } else if (view === "resultAccess") {
    await renderResultAccessControl();
  } else if (view === "bulkResults") {
    await renderBulkSessionResults();
  } else if (view === "carryApplications") {
    await renderCarryOverApplications();
  } else if (view === "browse") {
...
async function renderDashboard() {
  ...
  const content = `
    <section class="hero">
      <h2>${getCollegeConfig().name} — Administrator Dashboard</h2>
    ...
    <section class="grid stats-grid">
      ${statCard(state.counts.students, `${getCollegeConfig().key === "nursing" ? "Nursing" : "Health Science"} Students`)}
    ...
    <section class="section">
      ...
      <div class="grid quick-grid">
        ${quickAction("browse", "Browse Result Bank", "Open students by admission session and print official results.")}
        ...
        ${quickAction("studentAccounts", "Student Login Accounts", "Create personal passwords and restricted student accounts.")}
        ${quickAction("resultAccess", "Result Access Control", "Lock or unlock result access after fee clearance.")}
...
async function renderStudentAccounts() {
  dashboardLayout(`
    <section class="section-head">
      <div>
        ...
        <p>Create a separate Firebase login and a different password for every student.</p>
      </div>
      <button id="createMissingAccountsBtn" class="primary-btn">
    ...
    <section class="card">
      <div class="import-note">
        Passwords are generated when accounts are created and are also shown here for administrators.
        They are stored in the admin-only <code>studentCredentials</code> collection so they can be accessed later.
        Students cannot read this collection.
      </div>
      <div id="studentAccountsMessage"></div>
      <div id="studentAccountsArea" class="placeholder" style="margin-top:16px">
  ...
  document
    .getElementById("createMissingAccountsBtn")
    .addEventListener("click", createMissingStudentAccounts);

  await loadStudentAccountStatus();
}

async function loadStudentAccountStatus() {
  const area = document.getElementById("studentAccountsArea");

  try {
    const [snapshot, credentialsSnapshot] = await Promise.all([
      getDocs(collection(db, "students")),
      getDocs(collection(db, "studentCredentials"))
    ]);
    const credentials = new Map(credentialsSnapshot.docs.map(item => [item.id, item.data()]));
    const students = snapshot.docs
      .map(item => ({ id: item.id, ...item.data(), generatedPassword: credentials.get(item.id)?.generatedPassword || "" }))
      .filter(student => collegeMatches(student.department))
      .sort((a, b) => String(a.name || "").localeCompare(String(b.name || "")));

    const created = students.filter(student => student.authUid).length;
    const missing = students.length - created;

    area.className = "table-wrap";
    area.innerHTML = `
      <div class="preview-summary" style="padding:14px">
        <article><strong>${students.length}</strong><span>Total Students</span></article>
        <article><strong>${created}</strong><span>Accounts Created</span></article>
        <article><strong>${missing}</strong><span>Accounts Missing</span></article>
      </div>
        ...
        <tbody>
          ${students.map(student => `
            <tr>
              ...
              <td>${escapeHtml(collegeLabelForDepartment(student.department))}</td>
              <td><code>${escapeHtml(student.generatedPassword || "Not recorded")}</code></td>
              <td>
                <span class="status-badge ${student.authUid ? "active" : "closed"}">
                  ${student.authUid ? "Created" : "Not Created"}
                </span>
              </td>
            </tr>
          `).join("")}
...
async function createMissingStudentAccounts() {
  const button = document.getElementById("createMissingAccountsBtn");
  const message = document.getElementById("studentAccountsMessage");

  button.disabled = true;
  button.textContent = "Creating Accounts...";
  message.innerHTML = `<div class="message">Please keep this page open.</div>`;

  const secondaryAppName = `student-account-creator-${Date.now()}`;
  const secondaryApp = initializeApp(APP_CONFIG.firebase, secondaryAppName);
  const secondaryAuth = getAuth(secondaryApp);

  const credentials = [["Student Name", "Matric Number", "Login Password"]];
  let created = 0;
  let skipped = 0;
  const errors = [];
  ...
  try {
    ...
    for (const student of students) {
      ...
      const email = loginEmailForMatric(student.matricNumber);
      ...
      try {
        ...
        if (error.code === "auth/email-already-in-use") {
          ...
          errors.push(`${student.matricNumber}: ${error.message}`);
        }
      }
    }
    ...
    if (credentials.length > 1) {
      downloadCsv(
        `SAHLAN-Student-Passwords-${new Date().toISOString().slice(0, 10)}.csv`,
        credentials
      );
    }
    ...
    message.innerHTML = `
      <div class="message success">
        ${created} account(s) created. ${skipped} existing Firebase account(s) skipped.
        ${errors.length ? `${errors.length} account(s) could not be created.` : ""}
      </div>
      ${errors.length ? `<div class="message error">${errors.map(escapeHtml).join("<br>")}</div>` : ""}
    `;
    ...
    button.disabled = false;
    button.textContent = "Create Missing Accounts";
  }
}
...
async function loadStudentsAdmin() {
  ...
  try {
    ...
    students = students
      .filter(item => collegeMatches(item.department))
...
async function openStudentModal(studentId = null) {
  ...
  let student = {
    ...
    matricNumber: "",
    sex: "",
    department: state.activeCollege === "nursing" ? "NURSING" : "CHEW",
    admissionSession: "",
    currentLevel: 1
  };
        ...
        <form id="studentForm" class="form-grid">
          ...
          <div class="form-group">
            ...
            <select id="studentDepartment">
              ...
              <option value="NURSING" ${student.department === "NURSING" ? "selected" : ""}>Nursing & Midwifery</option>
            </select>
          </div>
...
function gradingEditorHtml(prefix, title, settings, isNursing=false) {
  const remarkTitle = isNursing ? "Carry-Over Remarks" : "CGPA Remarks";
  const remarkHint = isNursing
    ? "For Nursing & Midwifery, the final remark is based only on the number of outstanding carry-over courses."
    : "For Health Science & Technology, the final remark is based on CGPA.";
  const remarkRows = isNursing
    ? (settings.carryOverRemarks || []).map(band=>`<tr><td><input class="table-input ${prefix}-remark" value="${escapeHtml(band.remark)}"></td><td><input class="table-input ${prefix}-remark-min" type="number" min="0" value="${Number(band.min)}"></td><td><input class="table-input ${prefix}-remark-max" type="number" min="0" value="${Number(band.max)}"></td></tr>`).join("")
    : (settings.remarks || []).map(band=>`<tr><td><input class="table-input ${prefix}-remark" value="${escapeHtml(band.remark)}"></td><td><input class="table-input ${prefix}-remark-min" type="number" step="0.01" value="${Number(band.min)}"></td><td><input class="table-input ${prefix}-remark-max" type="number" step="0.01" value="${Number(band.max)}"></td></tr>`).join("");
  return `<section class="card grading-college-card"><h3>${escapeHtml(title)}</h3><p>${remarkHint}</p>
    <div class="form-grid"><div class="form-group"><label>${prefix} CGPA Scale</label><input id="${prefix}Scale" type="number" step="0.1" value="${Number(settings.scale||5)}"></div></div>
    <h4>Grade Bands</h4><div class="table-wrap"><table><thead><tr><th>Grade</th><th>Min</th><th>Max</th><th>Point</th><th>Status</th></tr></thead><tbody id="${prefix}GradeRows">
      ${settings.bands.map(band=>`<tr><td><input class="table-input ${prefix}-grade" value="${escapeHtml(band.grade)}"></td><td><input class="table-input ${prefix}-min" type="number" value="${Number(band.min)}"></td><td><input class="table-input ${prefix}-max" type="number" value="${Number(band.max)}"></td><td><input class="table-input ${prefix}-point" type="number" step="0.1" value="${Number(band.point)}"></td><td><select class="table-input ${prefix}-status"><option ${band.status==="Passed"?"selected":""}>Passed</option><option ${band.status==="Carry Over"?"selected":""}>Carry Over</option></select></td></tr>`).join("")}
    </tbody></table></div>
    <h4 style="margin-top:18px">${remarkTitle}</h4><div class="table-wrap"><table><thead><tr><th>Remark</th><th>Minimum ${isNursing ? "Carry Overs" : "CGPA"}</th><th>Maximum ${isNursing ? "Carry Overs" : "CGPA"}</th></tr></thead><tbody id="${prefix}RemarkRows">
      ${remarkRows}
    </tbody></table></div>
  </section>`;
}

function readGradingEditor(prefix, isNursing=false) {
  const bands=[...document.querySelectorAll(`.${prefix}-grade`)].map(input=>{const row=input.closest("tr");return {grade:input.value.trim().toUpperCase(),min:Number(row.querySelector(`.${prefix}-min`).value),max:Number(row.querySelector(`.${prefix}-max`).value),point:Number(row.querySelector(`.${prefix}-point`).value),status:row.querySelector(`.${prefix}-status`).value};});
  const remarks=[...document.querySelectorAll(`.${prefix}-remark`)].map(input=>{const row=input.closest("tr");return {remark:input.value.trim(),min:Number(row.querySelector(`.${prefix}-remark-min`).value),max:Number(row.querySelector(`.${prefix}-remark-max`).value)};});
  const base={scale:Number(document.getElementById(`${prefix}Scale`).value),bands};
...
function validateRemarkBands(remarks, scale) {
  ...
  const sorted=[...remarks].sort((a,b)=>a.min-b.min); for(let i=1;i<sorted.length;i++){if(sorted[i].min<=sorted[i-1].max)throw new Error(`CGPA remark ranges for ${sorted[i-1].remark} and ${sorted[i].remark} overlap.`);}
  if(sorted[0].min!==0||sorted[sorted.length-1].max!==Number(scale))throw new Error(`CGPA remarks must cover 0 to ${scale}.`);
}

function validateCarryOverRemarkBands(remarks) {
  if (!remarks.length) throw new Error("At least one Nursing carry-over remark is required.");
  remarks.forEach(r=>{if(!r.remark||r.min<0||r.max<r.min||!Number.isInteger(r.min)||!Number.isInteger(r.max))throw new Error(`Invalid carry-over remark range for ${r.remark||"blank remark"}.`);});
  const sorted=[...remarks].sort((a,b)=>a.min-b.min); for(let i=1;i<sorted.length;i++){if(sorted[i].min<=sorted[i-1].max)throw new Error(`Carry-over remark ranges for ${sorted[i-1].remark} and ${sorted[i].remark} overlap.`);}
  if(sorted[0].min!==0)throw new Error("Nursing carry-over remarks must start at 0 outstanding carry overs.");
}

async function renderSettings() {
  await loadGradingSettings();
  dashboardLayout(`<section class="section-head"><div><h3>College & Grading Settings</h3><p>Health Science uses CGPA-based remarks. Nursing & Midwifery uses carry-over-based remarks.</p></div></section>
    ${gradingEditorHtml("health", "SAHLAN COLLEGE OF HEALTH SCIENCE AND TECHNOLOGY, JOS", gradingSettings, false)}
    ${gradingEditorHtml("nursing", "SAHLAN COLLEGE OF NURSING AND MIDWIFERY", nursingGradingSettings, true)}
    <section class="card settings-actions"><button id="resetAllGradingBtn" class="secondary-btn">Restore Default Grading</button><button id="saveAllGradingBtn" class="primary-btn">Save Both Grading Systems</button><div id="settingsMessage" style="margin-top:12px"></div></section>`);
  document.getElementById("resetAllGradingBtn").addEventListener("click",()=>{gradingSettings=makeDefaultGrading();nursingGradingSettings=makeDefaultNursingGrading();renderSettings();});
  document.getElementById("saveAllGradingBtn").addEventListener("click",async()=>{const message=document.getElementById("settingsMessage");try{const health=readGradingEditor("health",false);const nursing=readGradingEditor("nursing",true);validateGradingBands(health.bands);validateGradingBands(nursing.bands);validateRemarkBands(health.remarks,health.scale);validateCarryOverRemarkBands(nursing.carryOverRemarks);await setDoc(doc(db,"settings","school"),{schoolName:APP_CONFIG.schoolName,gradingSettings:health,nursingGradingSettings:nursing,updatedAt:serverTimestamp()},{merge:true});gradingSettings=health;nursingGradingSettings=nursing;message.innerHTML='<div class="message success">Both grading systems and their remark rules were saved. Recalculate results to apply changes to existing records.</div>';}catch(error){message.innerHTML=`<div class="message error">${escapeHtml(error.message)}</div>`;}});
}

async function loadGradingSettings() {
  try { const snapshot=await getDoc(doc(db,"settings","school")); if(!snapshot.exists())return; const data=snapshot.data(); const old=data.gradingSettings||{}; gradingSettings={scale:Number(old.scale||data.cgpaScale||5),bands:Array.isArray(old.bands)&&old.bands.length?old.bands:structuredClone(DEFAULT_GRADING),remarks:Array.isArray(old.remarks)&&old.remarks.length?old.remarks:structuredClone(DEFAULT_REMARKS),remarkMode:"cgpa"}; const n=data.nursingGradingSettings||{}; nursingGradingSettings={scale:Number(n.scale||5),bands:Array.isArray(n.bands)&&n.bands.length?n.bands:structuredClone(DEFAULT_GRADING),remarks:[],carryOverRemarks:Array.isArray(n.carryOverRemarks)&&n.carryOverRemarks.length?n.carryOverRemarks:structuredClone(DEFAULT_NURSING_CARRYOVER_REMARKS),remarkMode:"carryovers"}; } catch(error){console.warn("Could not load grading settings:",error);}
}
...
async function loadSessionManagement() {
  ...
  try {
    ...
    studentSnapshot.docs.forEach(item => {
      if (!collegeMatches(item.data().department)) return;
    ...
    let sessions = sessionSnapshot.docs.map(item => ({ id: item.id, ...item.data() }))
      .filter(item => !item.department || collegeMatches(item.department));
...
async function renderCarryOverApplications() {
  dashboardLayout(`
    <section class="section-head">
      <div>
        <h3>Carry-Over Rewrite Applications</h3>
        <p>Review student applications and print the consolidated list.</p>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        <button id="printPendingApplicationsBtn" class="primary-btn">Print Pending List</button>
        <button id="approvedCourseListsBtn" class="secondary-btn">Approved Students by Course</button>
      </div>
    </section>
    ...
    <section class="card">
      ...
      <div id="carryApplicationsArea" class="placeholder" style="margin-top:16px">
  ...
  document
    ...
    .addEventListener("change", loadCarryOverApplications);

  document
    .getElementById("printPendingApplicationsBtn")
    .addEventListener("click", printPendingCarryApplications);

  document
    .getElementById("approvedCourseListsBtn")
    ?.addEventListener("click", showApprovedCarryOverCourses);

  await loadCarryOverApplications();
}

async function loadCarryOverApplications() {
  const area = document.getElementById("carryApplicationsArea");
  const status = document.getElementById("applicationStatusFilter")?.value || "";

  try {
    const snapshot = await getDocs(collection(db, "carryOverApplications"));
    ...
    if (!applications.length) {
      area.className = "placeholder";
      area.textContent = "No matching applications found.";
      return;
    }
        ...
        <tbody>
          ${applications.map(application => `
            <tr>
              ...
              <td>${escapeHtml(application.matricNumber || "")}</td>
              <td>${escapeHtml(application.department || "")}</td>
              <td>Year ${escapeHtml(application.currentLevel || "")}</td>
              <td>${(application.courses || []).map(course => escapeHtml(course.courseCode)).join(", ")}</td>
              <td>${escapeHtml(application.status || "Pending")}</td>
              <td class="action-cell">
                <button class="primary-btn approve-carry-btn" data-id="${application.id}">Approve</button>
                <button class="danger-btn reject-carry-btn" data-id="${application.id}">Reject</button>
              </td>
            </tr>
          `).join("")}
        </tbody>
    ...
    area.querySelectorAll(".approve-carry-btn").forEach(button => {
      button.addEventListener("click", () => updateCarryApplicationStatus(button.dataset.id, "Approved"));
    });

    area.querySelectorAll(".reject-carry-btn").forEach(button => {
      button.addEventListener("click", () => updateCarryApplicationStatus(button.dataset.id, "Rejected"));
    });
  } catch (error) {
    area.className = "message error";
    area.textContent = error.message;
...
async function updateCarryApplicationStatus(applicationId, status) {
  await setDoc(doc(db, "carryOverApplications", applicationId), {
    status,
...
async function printPendingCarryApplications() {
  ...
  if (!applications.length) {
    alert("There are no pending applications to print.");
    return;
  }
  ...
  container.id = "printOnlyContainer";
  container.className = "application-print-container";
  container.innerHTML = `
    <article class="official-result carry-application-print">
      ${officialResultHeader("CARRY-OVER REWRITE APPLICATION LIST")}
      <p class="application-print-date">
        Printed: ${new Date().toLocaleDateString("en-NG", { dateStyle: "long" })}
      </p>
      <table class="application-print-table">
        <thead>
          <tr>
        ...
        <tbody>
          ${applications.map((application, index) => `
            <tr>
              ...
              <td>${escapeHtml(application.department || "")}</td>
              <td>Year ${escapeHtml(application.currentLevel || "")}</td>
              <td>${(application.courses || []).map(course =>
...
async function showApprovedCarryOverCourses() {
  try {
    const snapshot = await getDocs(collection(db, "carryOverApplications"));
    const applications = snapshot.docs
      .map(item => ({ id: item.id, ...item.data() }))
    ...
    if (!applications.length) {
      alert("There are no approved carry-over applications for this college.");
      return;
    }

    const courseMap = new Map();
    applications.forEach(application => {
      (application.courses || []).forEach(course => {
        ...
        courseMap.get(code).students.push({
          name: application.studentName || "",
          matricNumber: application.matricNumber || "",
          department: application.department || "",
          level: application.currentLevel || "",
          session: application.resultSession || ""
        });
      });
    });

    const courses = [...courseMap.values()].sort((a,b) => a.courseCode.localeCompare(b.courseCode));

    dashboardLayout(`
      <section class="section-head">
        <div>
          <h3>Approved Carry-Over Students by Course</h3>
          <p>Only approved applications in the ${escapeHtml(getCollegeConfig().name)} view are shown.</p>
        </div>
        <button id="backCarryApplications" class="secondary-btn">← Back to Applications</button>
      </section>
      <section class="card">
        <div class="table-wrap">
          <table>
            <thead><tr><th>S/N</th><th>Course Code</th><th>Course Title</th><th>Approved Students</th><th>Action</th></tr></thead>
            <tbody>
              ${courses.map((course,index) => `
                <tr>
                  <td>${index+1}</td>
                  ...
                  <td><button class="primary-btn approved-course-print" data-code="${escapeHtml(course.courseCode)}">View / Print List</button></td>
                </tr>
              `).join("")}
            </tbody>
          </table>
    ...
    document.getElementById("backCarryApplications").addEventListener("click", () => navigate("carryApplications"));
    document.querySelectorAll(".approved-course-print").forEach(button => {
      button.addEventListener("click", () => {
        const course = courses.find(item => item.courseCode === button.dataset.code);
        if (course) printApprovedCarryOverCourse(course);
      });
    });
  } catch (error) {
    alert(error.message);
...
function printApprovedCarryOverCourse(course) {
  const students = [...course.students].sort((a,b) => String(a.name).localeCompare(String(b.name)));
  const container = document.createElement("div");
  container.id = "printOnlyContainer";
  container.className = "application-print-container";
  container.innerHTML = `
    <article class="official-result carry-application-print">
      ${officialResultHeader("APPROVED CARRY-OVER COURSE LIST")}
      <div class="course-list-heading"><strong>${escapeHtml(course.courseCode)}</strong> — ${escapeHtml(course.courseTitle || "")}</div>
      <p class="application-print-date">Total Approved Students: <strong>${students.length}</strong><br>Printed: ${new Date().toLocaleDateString("en-NG", {dateStyle:"long"})}</p>
      <table class="application-print-table"><thead><tr><th>S/N</th><th>Student Name</th><th>Matric Number</th><th>Department</th><th>Level</th><th>Session</th></tr></thead>
      <tbody>${students.map((student,index) => `<tr><td>${index+1}</td><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.matricNumber)}</td><td>${escapeHtml(collegeLabelForDepartment(student.department))}</td><td>Year ${escapeHtml(student.level)}</td><td>${escapeHtml(student.session)}</td></tr>`).join("")}</tbody></table>
      ${signatureSection()}
    </article>`;
  document.body.appendChild(container);
  document.body.classList.add("printing-result");
  const cleanup=()=>{document.body.classList.remove("printing-result");container.remove();window.removeEventListener("afterprint",cleanup)};
  window.addEventListener("afterprint",cleanup); window.print(); setTimeout(cleanup,3000);
}

async function renderResultAccessControl() {
  dashboardLayout(`
    <section class="section-head"><div><h3>Result Access Control</h3><p>Lock a student's result when school-fee clearance has not been confirmed.</p></div></section>
    <section class="card">
      <div class="form-grid"><div class="form-group full"><label for="accessStudentSearch">Search Student</label><input id="accessStudentSearch" placeholder="Name or matric number"></div></div>
      <div id="resultAccessArea" class="placeholder">Loading students...</div>
    </section>`);
  document.getElementById("accessStudentSearch").addEventListener("input", loadResultAccessStudents);
  await loadResultAccessStudents();
}

async function loadResultAccessStudents() {
  const area=document.getElementById("resultAccessArea"); if(!area)return;
  try {
    const snap=await getDocs(collection(db,"students"));
    const search=(document.getElementById("accessStudentSearch")?.value||"").trim().toLowerCase();
    const students=snap.docs.map(d=>({id:d.id,...d.data()})).filter(s=>collegeMatches(s.department)).filter(s=>!search||String(s.name||"").toLowerCase().includes(search)||String(s.matricNumber||"").toLowerCase().includes(search)).sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));
    area.className="table-wrap";
    area.innerHTML=students.length?`<table><thead><tr><th>Student</th><th>Matric Number</th><th>Department</th><th>Access</th><th>Action</th></tr></thead><tbody>${students.map(s=>`<tr><td>${escapeHtml(s.name)}</td><td>${escapeHtml(s.matricNumber)}</td><td>${escapeHtml(collegeLabelForDepartment(s.department))}</td><td><span class="status-badge ${s.resultLocked===true?"closed":"active"}">${s.resultLocked===true?"LOCKED":"ALLOWED"}</span></td><td><button class="${s.resultLocked===true?"primary-btn":"danger-btn"} toggle-result-lock" data-id="${escapeHtml(s.id)}" data-lock="${s.resultLocked===true}">${s.resultLocked===true?"Unlock Result":"Lock Result"}</button></td></tr>`).join("")}</tbody></table>`:`<div class="placeholder">No students found.</div>`;
    area.querySelectorAll(".toggle-result-lock").forEach(btn=>btn.addEventListener("click",()=>setResultLock(btn.dataset.id,btn.dataset.lock!=="true")));
  } catch(error){area.className="message error";area.textContent=error.message;}
}

async function setResultLock(studentId, locked) {
  const message = locked ? "Lock this student's result because school-fee clearance has not been confirmed?" : "Unlock this student's result?";
  if(!confirm(message)) return;
  try { await setDoc(doc(db,"students",studentId),{resultLocked:locked,resultLockUpdatedAt:serverTimestamp(),resultLockUpdatedBy:state.user?.email||""},{merge:true}); await loadResultAccessStudents(); }
  catch(error){alert(error.message);}
}

async function renderBulkSessionResults() {
  dashboardLayout(`
    <section class="section-head"><div><h3>Bulk Session Results</h3><p>Select a result session. Each student will occupy one A4 page containing First and Second Semester results.</p></div></section>
    <section class="card"><div class="form-grid"><div class="form-group"><label for="bulkSessionSelect">Result Session</label><select id="bulkSessionSelect"><option value="">Loading sessions...</option></select></div></div><div id="bulkResultMessage"></div><button id="downloadBulkResultsBtn" class="primary-btn" disabled>Download All Results as PDF</button></section>`);
  const snap=await getDocs(collection(db,"results"));
  const sessions=[...new Set(snap.docs.map(d=>d.data()).filter(r=>collegeMatches(r.department)).map(r=>String(r.resultSession||"").trim()).filter(Boolean))].sort().reverse();
  const select=document.getElementById("bulkSessionSelect"); select.innerHTML=sessions.length?sessions.map(s=>`<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join(""):`<option value="">No sessions found</option>`;
  const btn=document.getElementById("downloadBulkResultsBtn"); btn.disabled=!sessions.length; btn.addEventListener("click",()=>downloadBulkSessionPdf(select.value));
}

async function downloadBulkSessionPdf(session) {
  if(!window.jspdf||!window.html2canvas){alert("The PDF tools could not be loaded. Check your internet connection and refresh the portal.");return;}
  const message=document.getElementById("bulkResultMessage"); const btn=document.getElementById("downloadBulkResultsBtn"); btn.disabled=true; message.innerHTML='<div class="message">Preparing PDF. Please keep this page open...</div>';
  try {
    const [studentsSnap, resultsSnap]=await Promise.all([getDocs(collection(db,"students")),getDocs(collection(db,"results"))]);
    const students=studentsSnap.docs.map(d=>({id:d.id,...d.data()})).filter(s=>collegeMatches(s.department));
    const results=resultsSnap.docs.map(d=>({id:d.id,...d.data()})).filter(r=>collegeMatches(r.department)&&String(r.resultSession||"")===String(session));
    const byStudent=new Map(); results.forEach(r=>{const key=r.studentId||safeDocumentId(r.matricNumber);if(!byStudent.has(key))byStudent.set(key,[]);byStudent.get(key).push(r)});
    const rows=students.filter(s=>byStudent.has(s.id)).sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));
    if(!rows.length) throw new Error("No student results were found for this session.");
    const {jsPDF}=window.jspdf; const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4"});
    const stage=document.createElement("div"); stage.style.position="fixed";stage.style.left="-100000px";stage.style.top="0";stage.style.width="194mm";stage.style.background="#fff";document.body.appendChild(stage);
    for(let i=0;i<rows.length;i++){
      const student=rows[i]; const studentResults=byStudent.get(student.id).sort((a,b)=>resultSortKey(a)-resultSortKey(b));
      stage.innerHTML=`<article class="official-result session-result bulk-pdf-page">${officialResultHeader("ACADEMIC SESSION RESULT", student)}${studentIdentitySection(student)}${studentResults.map(semesterResultBlock).join("")}${yearSummaryBlock(studentResults)}${signatureSection()}</article>`;
      const canvas=await html2canvas(stage.querySelector(".official-result"),{scale:2,useCORS:true,backgroundColor:"#ffffff"});
      const img=canvas.toDataURL("image/jpeg",0.95); if(i>0)pdf.addPage(); pdf.addImage(img,"JPEG",8,8,194,281.94,undefined,"FAST");
      message.innerHTML=`<div class="message">Preparing ${i+1} of ${rows.length} student pages...</div>`;
    }
    stage.remove(); pdf.save(`SAHLAN-${getCollegeConfig().key === "nursing" ? "Nursing-Midwifery" : "Health-Science"}-${safeDocumentId(session)}-Results.pdf`); message.innerHTML='<div class="message success">PDF downloaded successfully.</div>';
  } catch(error){message.innerHTML=`<div class="message error">${escapeHtml(error.message)}</div>`;}
  ...
  dashboardLayout(`
      ...
      <form id="courseForm" class="form-grid">
        ...
        <div class="form-group">
          ...
          <select id="courseDepartment">
            <option value="CHEW" ${state.activeCollege === "health" ? "selected" : ""}>Community Health Extension Worker (CHEW)</option>
    ...
    <section class="card section">
      ...
      <label style="display:block;margin-top:12px">
        <input id="updateExistingCourses" type="checkbox">
        Update existing matching courses
      </label>
      <button id="importCoursesBtn" class="secondary-btn" style="margin-top:12px">Import Courses</button>
...
async function importCourseWorkbook() {
  ...
  const updateExisting = document.getElementById("updateExistingCourses").checked;
  ...
  try {
    ...
    rows.map(row => ({
      ...
      .forEach(course => uniqueMap.set(courseDocumentId(course), course));

    const mapped = [...uniqueMap.values()];

    if (!mapped.length) {
      throw new Error("No valid course rows were found.");
    }
    ...
    const coursesToWrite = mapped.filter(course => {
      const id = courseDocumentId(course);
      return updateExisting || !existingIds.has(id);
    });

    for (let start = 0; start < coursesToWrite.length; start += 400) {
      const batch = writeBatch(db);

      coursesToWrite.slice(start, start + 400).forEach(course => {
        batch.set(
          doc(db, "courses", courseDocumentId(course)),
    ...
    const skipped = mapped.length - coursesToWrite.length;

    message.innerHTML = `
      <div class="message success">
        ${coursesToWrite.length} new course(s) imported.
        ${skipped} existing matching course(s) skipped.
      </div>
    `;
...
async function cleanupDuplicateCourses() {
  ...
  snapshot.docs.forEach(item => {
    ...
    const canonicalId = courseDocumentId(data);
    if (!groups.has(canonicalId)) groups.set(canonicalId, []);
    groups.get(canonicalId).push({ id: item.id, data });
  });

  const duplicates = [];

  groups.forEach((items, canonicalId) => {
    items.forEach(item => {
      if (item.id !== canonicalId) duplicates.push(item.id);
    });
  });

  for (let start = 0; start < duplicates.length; start += 400) {
    const batch = writeBatch(db);
    duplicates.slice(start, start + 400).forEach(id => {
...
async function cleanupDuplicateResults() {
  ...
  snapshot.docs.forEach(item => {
    const data = item.data();
    const canonicalId = safeDocumentId(
      `${data.matricNumber}_${data.resultSession}_${data.level}_${data.semester}`
    );
  ...
  dashboardLayout(`
    ...
    <section class="card import-panel">
      ...
      <div class="form-grid">
        ...
        <div class="form-group full">
          <label>
            <input id="updateExistingResults" type="checkbox">
            Update existing matching student/result records
          </label>
          <small>Leave unchecked to import only new records and skip existing matches.</small>
        </div>
      </div>
...
async function saveImportToFirestore() {
  ...
  try {
    const { writes: allWrites, studentsWithoutRegistration } =
    ...
    const existingByCollection = {
      students: new Set((await getDocs(collection(db, "students"))).docs.map(item => item.id)),
      results: new Set((await getDocs(collection(db, "results"))).docs.map(item => item.id)),
      sessions: new Set((await getDocs(collection(db, "sessions"))).docs.map(item => item.id))
    };

    const writes = allWrites.filter(item => {
      if (updateExisting) return true;
      const collectionName = item.reference.parent.id;
      return !existingByCollection[collectionName]?.has(item.reference.id);
    });

    const skippedWrites = allWrites.length - writes.length;
    const chunkSize = 400;
    let completed = 0;
    ...
    button.textContent = "Removing duplicate records...";
    ...
    message.innerHTML = `
      <div class="message success">
        Import completed successfully. ${writes.length} new database record(s) saved and
        ${skippedWrites} existing matching record(s) skipped. GPA, cumulative CGPA,
        carry-overs and academic remarks were recalculated automatically.
      </div>
      ${studentsWithoutRegistration > 0 ? `
        <div class="message error" style="margin-top:8px">
          Warning: ${studentsWithoutRegistration} student(s) had no course registration on file
          for session ${escapeHtml(pendingImport.metadata.resultSession)}. Their results were
          saved with an empty course list until they register.
        </div>
      ` : ""}
    `;
...
async function buildFirestoreWrites(data) {
  ...
  let studentsWithoutRegistration = 0;
  ...
  for (const student of data.students) {
    ...
    const registeredCourseCodes = await getRegisteredCourseCodes(
      studentId,
      data.metadata.resultSession
    );

    if (!registeredCourseCodes) {
      studentsWithoutRegistration += 1;
    }

    const registeredCourses = filterCoursesToRegistration(
      student.courses,
      registeredCourseCodes
    );
    ...
    writes.push({
      ...
      data: {
        ...
        ...calculateSemesterSummary(registeredCourses, student.department),
        cgpa: null,
  ...
  if (studentsWithoutRegistration > 0) {
    console.warn(
      `${studentsWithoutRegistration} student(s) had no course registration on file for session ${data.metadata.resultSession}. Their results were saved with an empty course list.`
    );
  }

  return { writes, studentsWithoutRegistration };
}
...
const DEFAULT_REMARKS = [
  { min: 3.50, max: 5.00, remark: "Distinction" },
  { min: 3.00, max: 3.49, remark: "Upper Credit" },
  { min: 2.50, max: 2.99, remark: "Lower Credit" },
  { min: 2.00, max: 2.49, remark: "Pass" },
  { min: 0.00, max: 1.99, remark: "Probation" }
];
// Nursing & Midwifery remarks are intentionally based on outstanding carry-over count, not CGPA.
// These defaults are editable by the Administrator in School Settings.
const DEFAULT_NURSING_CARRYOVER_REMARKS = [
  { min: 0, max: 0, remark: "Promoted" },
  { min: 1, max: 2, remark: "Probation" },
  { min: 3, max: 999, remark: "Repeat" }
];
function makeDefaultGrading(){return {scale:5,bands:structuredClone(DEFAULT_GRADING),remarks:structuredClone(DEFAULT_REMARKS),remarkMode:"cgpa"};}
function makeDefaultNursingGrading(){return {scale:5,bands:structuredClone(DEFAULT_GRADING),remarks:[],carryOverRemarks:structuredClone(DEFAULT_NURSING_CARRYOVER_REMARKS),remarkMode:"carryovers"};}
let gradingSettings=makeDefaultGrading();
let nursingGradingSettings=makeDefaultNursingGrading();
function gradingForDepartment(department){return collegeKeyForDepartment(department)==="nursing"?nursingGradingSettings:gradingSettings;}
function gradeFromScore(score, department=null){
  ...
  try {
    ...
    grouped.forEach(studentResults => {
      ...
      studentResults.forEach(result => {
        const courseRows = calculateCourseRows(result.courses || [], result.department);
        const summary = calculateSemesterSummary(courseRows, result.department);
        ...
        let remark = academicRemark(cgpa, result.department, unresolved.length);

        if (collegeKeyForDepartment(result.department) !== "nursing" && isSecondSemester(result.semester)) {
          if (isProbationStanding(cgpa, result.department)) {
            consecutiveProbationYears += 1;
            if (consecutiveProbationYears >= 2) remark = "Repeat";
          } else {
...
async function openResultEditor(resultId) {
  ...
  document.getElementById("saveEditedResult").addEventListener("click", async () => {
    ...
    try {
      const calculatedRows = calculateCourseRows(courses, result.department);
      const summary = calculateSemesterSummary(calculatedRows, result.department);
...
async function recalculateSingleStudent(studentId) {
  ...
  await updateStudentCurrentLevel(studentId, studentResults);
}
            ...
            <tbody>
              ${(result.courses || []).map((course, index) => `
                <tr>
                  ...
                  <td>${escapeHtml(course.grade || gradeFromScore(course.score, result.department).grade)}</td>
                  <td>${Number(course.gradePoint ?? gradeFromScore(course.score, result.department).point)}</td>
                  <td>${Number(course.creditPoint ?? (Number(course.units || 0) * gradeFromScore(course.score, result.department).point))}</td>
                  <td>${escapeHtml(course.status || gradeFromScore(course.score, result.department).status)}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
...
async function printResultDocument(mode = "semester") {
  if (state.userProfile?.role !== "admin") {
    alert("Only the Administrator is authorized to print official results.");
    return;
  }
  ...
  printDocument.write(`
    ...
    <html lang="en">
      <head>
        ...
        <link
          rel="stylesheet"
          href="./assets/css/style.css?v=4.1.0"
        >

        <link
          rel="stylesheet"
          href="./assets/css/print.css?v=4.1.0"
        >
      </head>
    ...
    <section class="card">
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select id="deptFilter"><option value="">All</option><option>CHEW</option><option>MLT</option><option value="NURSING">Nursing & Midwifery</option></select>
        </div>
      </div>
...
async function loadStudentsBySession(session) {
  ...
  const rows = snap.docs.map(d => ({id:d.id, ...d.data()}))
    .filter(s => collegeMatches(s.department))
    .filter(s => s.admissionSession === session)
...
const years = [...new Set(results.map(r => Number(r.level)).filter(Boolean))].sort();
  ...
  dashboardLayout(`
    ...
    <section class="card section">
      ...
      <div class="form-grid">
        <div class="form-group">
          <label for="adminRegistrationSession">Academic Session</label>
          <input id="adminRegistrationSession" value="${escapeHtml(getCurrentAcademicSession())}" placeholder="Example: 2024/2025">
        </div>
        <div class="form-group">
          <label>&nbsp;</label>
          <button id="loadAdminRegistrationBtn" class="secondary-btn">Load Registration</button>
        </div>
      </div>

      <div id="adminCourseRegistrationArea"></div>
    </section>
  ...
  document.getElementById("loadAdminRegistrationBtn").addEventListener("click", () => renderAdminCourseRegistration(student));
  renderAdminCourseRegistration(student);
...
async function renderAdminCourseRegistration(student) {
  const area = document.getElementById("adminCourseRegistrationArea");
  const session = document.getElementById("adminRegistrationSession").value.trim();

  if (!session) {
    area.innerHTML = `<div class="message error">Enter an academic session.</div>`;
    return;
  }
  ...
  let registration;
  try {
    registration = await getStudentCourseRegistration(student.id, session);
  } catch (error) {
    area.innerHTML = `<div class="message error">${escapeHtml(error.message)}</div>`;
    return;
  }
  ...
  if (!registration) {
    area.innerHTML = `
      <div class="message">
        No course registration on file for ${escapeHtml(student.name)} in ${escapeHtml(session)}.
      </div>
    `;
    return;
  }

  const maximumCreditLoad =
    getMaximumCreditLoad(student.department, registration.level) ||
    Number(registration.maximumCreditLoad || 0);
  ...
  const carryOverLoad = calculateRegistrationCredits(
    carrySnapshot.docs.flatMap(item => item.data().courses || [])
  );

  const newCourseCreditCap = Math.max(0, maximumCreditLoad - carryOverLoad);

  const courseSnapshot = await getDocs(collection(db, "courses"));
  const allCourses = courseSnapshot.docs
    .map(item => ({ id: item.id, ...item.data() }))
    .filter(course =>
      String(course.department || "").trim().toUpperCase() ===
      String(student.department || "").trim().toUpperCase()
    )
    .filter(course => Number(course.level) === Number(registration.level))
    .sort((a, b) => {
      const semesterA = String(a.semester || "").toLowerCase().startsWith("first") ? 1 : 2;
      const semesterB = String(b.semester || "").toLowerCase().startsWith("first") ? 1 : 2;
      return semesterA - semesterB ||
        String(a.courseCode || "").localeCompare(String(b.courseCode || ""));
    });

  const registeredCodes = new Set(
    (registration.courses || []).map(course => normalizeCourseCode(course.courseCode))
  );

  area.innerHTML = `
    <div class="credit-load-box">
      <strong>New Courses Selected:</strong>
      <span id="adminSelectedCreditLoad">${calculateRegistrationCredits(registration.courses)}</span>
      / <span>${newCourseCreditCap}</span> Units available
      ${carryOverLoad ? `<br><small>Plus ${carryOverLoad} carry-over unit(s) already committed. Combined total may not exceed ${maximumCreditLoad} units.</small>` : ""}
    </div>
          ...
          ${allCourses.map((course, index) => `
            <tr>
              <td>
                <input
                  type="checkbox"
                  class="admin-registration-course-checkbox"
                  data-course-id="${escapeHtml(course.id)}"
                  data-code="${escapeHtml(course.courseCode)}"
                  data-title="${escapeHtml(course.courseTitle)}"
                  data-unit="${Number(course.units || 0)}"
                  data-semester="${escapeHtml(course.semester)}"
                  ${registeredCodes.has(normalizeCourseCode(course.courseCode)) ? "checked" : ""}
                >
              </td>
              <td>${index + 1}</td>
              <td>${escapeHtml(course.courseCode)}</td>
    ...
    <button id="saveAdminRegistrationBtn" class="primary-btn" style="margin-top:12px">Save Corrections</button>
  `;

  const updateSelectedLoad = () => {
    const checked = [...document.querySelectorAll(".admin-registration-course-checkbox:checked")];
    const total = checked.reduce((sum, box) => sum + Number(box.dataset.unit || 0), 0);
    document.getElementById("adminSelectedCreditLoad").textContent = total;
  };

  document.querySelectorAll(".admin-registration-course-checkbox").forEach(box => {
    box.addEventListener("change", updateSelectedLoad);
  });

  document.getElementById("saveAdminRegistrationBtn").addEventListener("click", async () => {
    const button = document.getElementById("saveAdminRegistrationBtn");
    const message = document.getElementById("adminRegistrationMessage");

    const checked = [...document.querySelectorAll(".admin-registration-course-checkbox:checked")];
    const courses = checked.map(box => ({
      courseId: box.dataset.courseId,
      courseCode: box.dataset.code,
      courseTitle: box.dataset.title,
      units: Number(box.dataset.unit || 0),
    ...
    const totalCreditLoad = calculateRegistrationCredits(courses);

    if (newCourseCreditCap && totalCreditLoad > newCourseCreditCap) {
      const proceed = confirm(
        `Selected credit load is ${totalCreditLoad} units, above the ${newCourseCreditCap} unit(s) available for new courses (maximum ${maximumCreditLoad}, minus ${carryOverLoad} carry-over unit(s) already committed). Save anyway?`
      );
      if (!proceed) return;
    }
    ...
    try {
      const registrationId = getRegistrationDocumentId(student.id, session);
      await setDoc(
        doc(db, "courseRegistrations", registrationId),
        {
          courses,
      ...
      message.innerHTML = `
        <div class="message success">
          Saved. The student can now re-download their registration form with these corrections.
        </div>
      `;
...
function officialResultHeader(title, subject = null) {
  const college = getCollegeConfig(subject);
  return `<header class="result-header"><img src="${college.logo}" alt="School logo"><div><h1>${escapeHtml(college.name)}</h1><h2>${escapeHtml(title)}</h2></div></header>`;
}
  ...
  return `<section class="semester-block">
    ...
    <div class="table-wrap result-table-wrap"><table><thead><tr>
      ...
      ${(result.courses || []).map((c,i) => `<tr>
        ...
        <td>${Number(c.score||0)}</td><td>${escapeHtml(c.grade || gradeFromScore(c.score, result.department).grade)}</td>
        <td>${Number(c.creditPoint ?? 0)}</td><td>${escapeHtml(c.status || gradeFromScore(c.score, result.department).status)}</td>
      </tr>`).join("")}
    </tbody></table></div>
...
function signatureSection() {
  return `<section class="signature-grid single-signature">
    <div><span class="signature-space"></span><strong>Registrar</strong></div>
  </section>`;
}

function courseRegistrationSignatureSection() {
  return `<section class="signature-grid">
    <div><span class="signature-space"></span><strong>Student's Signature / Date</strong></div>
    <div><span class="signature-space"></span><strong>Exam Officer / Level Coordinator</strong></div>
  </section>`;
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    const profile = await getSignedInUserProfile(user);
    state.userProfile = profile;

    if (!profile) {
      throw new Error("This login account is not linked to a system user profile.");
    }
    ...
    throw new Error("This account does not have an allowed role.");
  } catch (error) {
    ...
    app.innerHTML = `
      <main class="login-screen">
        <section class="login-card">
          ...
          <button id="verificationLogoutBtn" class="primary-btn" style="width:100%;margin-top:18px">
            Return to Home
          </button>
        </section>
      </main>
    ...
    document
      .getElementById("verificationLogoutBtn")
